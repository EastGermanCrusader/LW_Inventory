...
cfg = LW_SHIELD.cfg();
cfg._configured = LW_SHIELD.configured !== false && !!cfg.loginHash;
cfg.publicApiUrl = cfg._configured ? LW_SHIELD.publicApiUrl() : '';
cfg.apiUrl = null;
try {
...
try {
cfg.apiUrl = await LW_SHIELD.unlock(password);
cfg._loginToken = password;
sessionStorage.setItem('lw_url', cfg.apiUrl);
return true;
} catch (e) {
console.error(e);
return false;
...
if (window.LW_ERRORS) {
LW_ERRORS.log('error', msg, opts || { source: 'app' });
}
}
function logErrorFromErr(err, opts) {
if (window.LW_ERRORS) {
LW_ERRORS.logFromError(err, opts || { source: 'app' });
}
}
...
if (window.LW_SPLASH) {
if (msg) LW_SPLASH.setStatus(msg);
if (pct != null) LW_SPLASH.setProgress(pct);
}
}
function splashDone() {
if (window.LW_SPLASH) LW_SPLASH.finish();
}
async function boot() {
try {
